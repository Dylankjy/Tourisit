...
def bookings():
    try:
        ...
        # if not logged in
        if not result:
            return render_template('customer/booking.html', process_step=6, loggedin=False)
        # if logged in
        else:
            return render_template('customer/booking.html', process_step=6, loggedin=True, user=result)
    except:
        return 'Error trying to render'
